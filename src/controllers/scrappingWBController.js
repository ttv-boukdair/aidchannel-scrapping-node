...
async function getInfoProject(link) {
	...
	let info = await page.evaluate((link) => {
		...
		// if thematique is not set do nothing
		if (thematique == null) {
			thematique = null;
		}
		// else if there is one thematique get it
		...
		// else get all thematiques as a table
		else {
		...
		//country_id and other straight forward data processing
		...
		// get first funder as funder
		var funder = funder_orgs ? funder_orgs[0] : null
		// if funder is not null remove it fom list
		if (funder) funder_orgs.splice(0, 1)
		// get the rest as sub_funders
        ...
        // form project field and return it
		let project_to_save = {
			source: source,
			source_id: "44000-" + proj_org_id,
			project_url:link,
...
//puppeteer function to bypass JS issues in WB site
async function getProLink(wburl) {
	...
	//fetch projects WB codes from page and form urls
	let data = await page.evaluate(() => {
		//get all projects WB codes
		var project_ids = document.querySelectorAll("td[data-th='Project ID:']");
		// form urls to individual project pages
					...
					if (!maj_recent) {
						...
						if (counts) {
							for (let c = 0; c < country_id.length; c++) {
								var project_to_save = await ProjectPreProd.updateOne({ proj_org_id: proj_org_id, country: country_id[c] }, {
									$set: {
							...
							var project_to_save = await ProjectPreProd.updateOne({ proj_org_id: proj_org_id, country: country_id }, {
								$set: {
									source: project.source,
									source_id: project.source_id,
									proj_org_id: proj_org_id,
...
exports.newWBProjects = async (req, res, next) => {
	//number of documents
	let n_docs = 10000
	//project to skip per page
	let offset = 0
	// bool value to stop if condiition is met
	let to_stop = false
	//loop througth pages
	for (let p = 0; p < n_docs; p++) {
		//set offset starting with 0
		offset = p * 20
		// if condition is met in internal loop this value will be true, breaking his external loop
		if (to_stop) break
		//base link with 20 projects per page (the page is slow to load so 20 is the sweet spot)
		...
		for (var i = 0; i < links.length; i++) {
			try {
				...
				//else add new peoject
					...
					//sub_funder_id
					let sub_funder = await getSubFunder(project.sub_funder)
					//task_manager not used for now
					let user = await get_user(project.task_manager)
					//thematique
					let thematique = await thematique_norm(project.thematique)
					// if multinaltional duplicate project
					if (counts) {
						for (let c = 0; c < country_id.length; c++) {
