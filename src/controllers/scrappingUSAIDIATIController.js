...
let status_ongoing = "activity-status=1|2&"
const url_iati_base = "https://datastore.codeforiati.org/api/1/access/activity.json?reporting-org=US-GOV-1&recipient-country=MA&"
...
exports.newUSAID = async (req, res, next) => {
  //number of docs to load per page
  const max = 100
  //pagination by skipping projects
  var offset = 0
  //number of pages
  const n_docs = await get_docs_number()
  //stopping bool
  ...
  for (let i = 0; i < 1; i++) {
    //stop if condition is met in internal loop
    if (to_stop) {
      to_stop = true
      break
    }
    //num projects to skip
    offset = i * max
    //url with max projects
    const url_iati = url_iati_base + "limit=" + max + "&offset=" + offset
    console.log(url_iati)
    //get page of url_iati
    let response = await axios.get(url_iati, {
      headers: { 'User-Agent': 'Axios 0.21.1' }
    });
    //get page's projects
    ...
    for (let j = 0; j < 10; j++) {
      try {
        ...
        //chack if proj exists
        let proj_exist = await containsProject(source_id)
        //if so break and set condition bool to true
...
function iati_descriptions(project) {
  ...
  try {
    ...
    if (isString(project.description.narrative.text)) { description = project.description.narrative.text }
  }
  catch (e) { }
