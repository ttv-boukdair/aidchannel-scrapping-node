...
exports.interrupted = async (req, res, next) => {
  const interr = await Params.find({source_id:"IATIv2"})
...
function iati_descriptions(project){
  ...
  try{
  if(isString(project.description.narrative)){ description = project.description.narrative}
  else{
     try{
...
exports.scrappingIATI = async (req, res, next) => {
    ...
    for(let j=0; j< projects.length; j++){
      ...
      iati_org = tmp[1]}
          ...
          var status = await iati_status_norm(status_code);
            ...
            if( proj_exist == false){
            if(!countries_ids) countries_ids = []
            if(countries_ids.length>0){
              for(let i = 0 ; i<countries_ids.length; i++){
              let project_to_save =new ProjectPreProd({ source_id: source_id,
              name: name,
              funder: funder,
  ...
  await Params.updateOne({source_id:"IATIv2"}, {$set:{ interrupted:true, multinational:false,row:j,offset: offset}})
      ...
      if(countries_ids.length>0){
        ...
        if(proj_c.length){
        project_to_save=await ProjectPreProd.updateOne({ source_id: source_id, country: countries_ids[i]._id,},{$set:{ source_id: source_id,
        name: name,
        funder: funder,
        implemeter: implementer,
        sub_funder: sub_funder,
      ...
      project_to_save=new ProjectPreProd({ source_id: source_id,
        name: name,
        funder: funder,
        implemeter: implementer,
        sub_funder: sub_funder,
  ...
  await Params.updateOne({source_id:"IATIv2"}, {$set:{ interrupted:true, multinational:true,row:j,offset: offset}})}
    ...
    let project_to_save =await ProjectPreProd.updateOne({ source_id: source_id},{$set:{ source_id: source_id,
      name: name,
      funder: funder,
      implemeter: implementer,
      sub_funder: sub_funder,
...
async function iati_status_norm(status_code) {
  statuses = [['Pipeline/identification', '1'], ['Implementation', '2'], ['Finalisation', '3'], ['Closed', '4'], ['Cancelled', '5'], ['Suspended', '6'], ['completed', '3'], ['closed', '4'], ['identification', '1'], ['approved', ''], ['lending', ''], ['ongoing', '2'], ['cancelled', '5'], ['suspended', '6']]
  let status_id = null;
  var status_tmp = null;
...
async function get_user(user) {
  ...
  if(user['person-name']) {fullname = user['person-name'].narrative
                            if(isString(fullname) == false && fullname!=null) fullname=fullname.text}
  if(user['mailing-address']) {address = user['mailing-address'].narrative
                            if(isString(address) == false && address!=null) address=address.text}
  if(user['job-title']) {job_title = user['job-title'].narrative
